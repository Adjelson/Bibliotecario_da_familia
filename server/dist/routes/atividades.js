...
/* =========================== Upload =========================== */
const UPLOAD_DIR = path_1.default.join(process.cwd(), 'uploads', 'eventos');
...
/* =========================== Schemas =========================== */
const StatusEvt = zod_1.z.enum(['agendada', 'em_andamento', 'concluida']);
const EventoBodyBase = zod_1.z.object({
    titulo: zod_1.z.string().trim().min(1, 'Título é obrigatório'),
    descricao: zod_1.z.string().trim().min(1, 'Descrição é obrigatória'),
    data: zod_1.z.coerce.date(),
    horario: zod_1.z
        .string()
        .regex(/^\d{2}:\d{2}\s-\s\d{2}:\d{2}$/, 'Formato: "HH:MM - HH:MM"'),
    local: zod_1.z.string().trim().min(1, 'Local é obrigatório'),
    vagas: zod_1.z.coerce.number().int().min(1, 'Vagas mínimas: 1'),
    status: StatusEvt.default('agendada'),
    imagem: zod_1.z.string().min(1).optional(),
});
...
const InscricaoBody = zod_1.z.object({
    familiaId: zod_1.z.coerce.number().int().positive().optional(),
    utilizadorId: zod_1.z.coerce.number().int().positive().optional(),
    todosFamilia: zod_1.z.boolean().optional(),
    incluirResponsavel: zod_1.z.boolean().optional(),
    filhosIds: zod_1.z.array(zod_1.z.coerce.number().int().positive()).optional(),
    numFilhosAcompanhantes: zod_1.z.coerce.number().int().min(0).optional(),
});
/* =========================== Helpers =========================== */
function ymdLocal(d) {
...
async function getUserScope(req) {
    ...
    if (!authCtx?.userId || !authCtx?.role)
        throw httpError(401, 'Não autenticado');
    const isAdmin = authCtx.role === client_1.Role.ADMIN;
    const isBibliotecario = authCtx.role === client_1.Role.BIBLIOTECARIO;
    const isPai = authCtx.role === client_1.Role.PAI;
    if (isAdmin) {
    ...
    if (!u)
        throw httpError(401, 'Utilizador inválido');
...
async function getSumsEvento(eventoId) {
    const g = await prisma_1.prisma.eventoParticipante.groupBy({
        by: ['eventoId'],
        where: { eventoId },
...
async function getSumsEventos(ids) {
    if (!ids.length)
        return new Map();
    const g = await prisma_1.prisma.eventoParticipante.groupBy({
        by: ['eventoId'],
    ...
    ids.forEach(id => { if (!map.has(id))
        map.set(id, { adultos: 0, filhos: 0, total: 0 }); });
    return map;
}
async function getPresentesMap(ids) {
    if (!ids.length)
        return new Map();
    const g = await prisma_1.prisma.eventoParticipante.groupBy({
        by: ['eventoId'],
 ...
 */
r.get('/', (0, auth_1.auth)(), (0, auth_1.requireRole)(client_1.Role.PAI, client_1.Role.BIBLIOTECARIO, client_1.Role.ADMIN), (0, async_1.asyncHandler)(async (req, res) => {
    const { isAdmin, bibliotecaId: myBibId } = await getUserScope(req);
    const q = req.query.q?.trim() ?? '';
    const tempo = req.query.tempo?.trim() || 'todas';
    ...
    if (from && to)
        where.data = { gte: from, lte: endOfDay(to) };
    else if (from)
        where.data = { gte: from };
    else if (to)
        where.data = { lte: endOfDay(to) };
    else {
        if (tempo === 'hoje')
            where.data = { gte: hoje, lte: endOfDay(hoje) };
        else if (tempo === 'futuras')
            where.data = { gt: endOfDay(hoje) };
        else if (tempo === 'passadas')
            where.data = { lt: hoje };
    }
    ...
    if (authCtx && ids.length) {
        ...
        const whereIns = { eventoId: { in: ids } };
        if (familiaId)
            whereIns.familiaId = familiaId;
        else
            whereIns.utilizadorId = authCtx.userId;
        const mine = await prisma_1.prisma.eventoParticipante.findMany({
            where: whereIns,
    ...
    res.json({
        items: items.map(e => toDto(req, e, sumsMap.get(e.id) || { adultos: 0, filhos: 0, total: 0 }, { ...(inscricoesMap.get(e.id) ?? { inscrito: false, presente: false }) }, presentesMap.get(e.id) ?? 0)),
        total,
        page,
        pageSize,
    });
...
/* =========================== DETALHE =========================== */
r.get('/:id', (0, auth_1.auth)(), (0, auth_1.requireRole)(client_1.Role.PAI, client_1.Role.BIBLIOTECARIO, client_1.Role.ADMIN), (0, async_1.asyncHandler)(async (req, res) => {
    const { isAdmin, bibliotecaId: myBibId } = await getUserScope(req);
    const id = zod_1.z.coerce.number().int().positive().parse(req.params.id);
    const e = await prisma_1.prisma.evento.findUnique({ where: { id } });
    if (!e)
    ...
    const presentes = await prisma_1.prisma.eventoParticipante.count({
    ...
    if (authCtx) {
        const familiaId = await resolveFamiliaIdForUser(authCtx.userId);
        const whereIns = { eventoId: id };
        if (familiaId)
            whereIns.familiaId = familiaId;
        else
            whereIns.utilizadorId = authCtx.userId;
        const ex = await prisma_1.prisma.eventoParticipante.findFirst({
            where: whereIns,
...
/* =========================== CRIAR =========================== */
r.post('/', (0, auth_1.auth)(), (0, auth_1.requireRole)(client_1.Role.ADMIN, client_1.Role.BIBLIOTECARIO), (0, async_1.asyncHandler)(async (req, res) => {
    const { isAdmin, bibliotecaId: myBibId } = await getUserScope(req);
    ...
    const created = await prisma_1.prisma.evento.create({
        data: {
            titulo: body.titulo,
    ...
    res
        .status(201)
        .json(toDto(req, created, { adultos: 0, filhos: 0, total: 0 }, { inscrito: false, presente: false }, 0));
}));
/* =========================== UPDATE (full) =========================== */
r.put('/:id', (0, auth_1.auth)(), (0, auth_1.requireRole)(client_1.Role.ADMIN, client_1.Role.BIBLIOTECARIO), (0, async_1.asyncHandler)(async (req, res) => {
    const { isAdmin, bibliotecaId: myBibId } = await getUserScope(req);
    const id = zod_1.z.coerce.number().int().positive().parse(req.params.id);
    const body = EventoCreateBody.parse(req.body);
    const current = await prisma_1.prisma.evento.findUnique({ where: { id } });
    if (!current)
        return res.status(404).json({ message: 'Evento não encontrado' });
    ...
    const up = await prisma_1.prisma.evento.update({
        where: { id },
        data: {
            titulo: body.titulo,
            descricao: body.descricao,
    ...
    const [sums, presentes] = await Promise.all([
        getSumsEvento(id),
        prisma_1.prisma.eventoParticipante.count({ where: { eventoId: id, presente: true } }),
    ]);
...
r.patch('/:id', (0, auth_1.auth)(), (0, auth_1.requireRole)(client_1.Role.ADMIN, client_1.Role.BIBLIOTECARIO), (0, async_1.asyncHandler)(async (req, res) => {
    ...
    const up = await prisma_1.prisma.evento.update({ where: { id }, data });
    const [sums, presentes] = await Promise.all([
        getSumsEvento(id),
        prisma_1.prisma.eventoParticipante.count({ where: { eventoId: id, presente: true } }),
    ]);
...
/* =========================== DELETE =========================== */
r.delete('/:id', (0, auth_1.auth)(), (0, auth_1.requireRole)(client_1.Role.ADMIN, client_1.Role.BIBLIOTECARIO), (0, async_1.asyncHandler)(async (req, res) => {
    const { isAdmin, bibliotecaId: myBibId } = await getUserScope(req);
    const id = zod_1.z.coerce.number().int().positive().parse(req.params.id);
    const evt = await prisma_1.prisma.evento.findUnique({ where: { id } });
    if (!evt)
        return res.status(404).json({ message: 'Evento não encontrado' });
    if (!isAdmin && evt.bibliotecaId !== myBibId) {
        return res.status(403).json({ message: 'Sem permissão' });
    }
    await prisma_1.prisma.eventoParticipante.deleteMany({ where: { eventoId: id } });
    await prisma_1.prisma.evento.delete({ where: { id } });
    res.status(204).send();
}));
/* =========================== Upload de imagem =========================== */
r.post('/:id/imagem', (0, auth_1.auth)(), (0, auth_1.requireRole)(client_1.Role.ADMIN, client_1.Role.BIBLIOTECARIO), upload.single('file'), (0, async_1.asyncHandler)(async (req, res) => {
    const { isAdmin, bibliotecaId: myBibId } = await getUserScope(req);
    const id = zod_1.z.coerce.number().int().positive().parse(req.params.id);
    if (!req.file)
        return res.status(400).json({ message: 'Ficheiro obrigatório (field: file)' });
    const evt = await prisma_1.prisma.evento.findUnique({ where: { id } });
    if (!evt)
        return res.status(404).json({ message: 'Evento não encontrado' });
    if (!isAdmin && evt.bibliotecaId !== myBibId) {
    ...
    const up = await prisma_1.prisma.evento.update({ where: { id }, data: { imagem: relPath } });
    const [sums, presentes] = await Promise.all([
        getSumsEvento(id),
        prisma_1.prisma.eventoParticipante.count({ where: { eventoId: id, presente: true } }),
    ]);
...
/* =========================== INSCRIÇÕES =========================== */
r.post('/:id/inscricoes', (0, auth_1.auth)(), (0, auth_1.requireRole)(client_1.Role.PAI, client_1.Role.ADMIN, client_1.Role.BIBLIOTECARIO), (0, async_1.asyncHandler)(async (req, res) => {
    const eventoId = zod_1.z.coerce.number().int().positive().parse(req.params.id);
    ...
    if (!familiaId && !utilizadorId) {
        ...
        if (famId)
            ...
            utilizadorId = authCtx.userId;
    }
    const evento = await prisma_1.prisma.evento.findUnique({ where: { id: eventoId } });
    if (!evento)
        return res.status(404).json({ message: 'Evento não encontrado' });
    let familiaUserId = null;
    let filhosValidos = [];
    if (familiaId) {
    ...
    const existentes = await prisma_1.prisma.eventoParticipante.findMany({
        where: {
            eventoId,
            OR: [
    ...
    if (novoTotal <= 0)
        return res.status(400).json({ message: 'Inscrição vazia.' });
    ...
    if (novoTotal > disponivel)
        return res.status(409).json({ message: 'Evento lotado', disponivel });
    ...
    const txResult = await prisma_1.prisma.$transaction(async (tx) => {
        ...
        if (keptId) {
            ...
            for (const ex of existentes) {
                if (ex.id !== keptId)
                    await tx.eventoParticipante.delete({ where: { id: ex.id } });
            }
        }
        else {
    ...
    const presentes = await prisma_1.prisma.eventoParticipante.count({ where: { eventoId, presente: true } });
    const participante = await prisma_1.prisma.eventoParticipante.findUnique({ where: { id: txResult.keptId } });
...
r.delete('/:id/inscricoes', (0, auth_1.auth)(), (0, auth_1.requireRole)(client_1.Role.PAI, client_1.Role.BIBLIOTECARIO, client_1.Role.ADMIN), (0, async_1.asyncHandler)(async (req, res) => {
    const eventoId = zod_1.z.coerce.number().int().positive().parse(req.params.id);
    ...
    const ex = await prisma_1.prisma.eventoParticipante.findFirst({ where: whereMine, select: { id: true } });
    if (!ex)
        return res.status(404).json({ message: 'Inscrição não encontrada.' });
    await prisma_1.prisma.eventoParticipante.delete({ where: { id: ex.id } });
    res.status(204).send();
}));
/* =========================== Listar inscrições (staff) =========================== */
r.get('/:id/inscricoes', (0, auth_1.auth)(), (0, auth_1.requireRole)(client_1.Role.ADMIN, client_1.Role.BIBLIOTECARIO), (0, async_1.asyncHandler)(async (req, res) => {
    const { isAdmin, bibliotecaId: myBibId } = await getUserScope(req);
    const eventoId = zod_1.z.coerce.number().int().positive().parse(req.params.id);
    const evt = await prisma_1.prisma.evento.findUnique({ where: { id: eventoId } });
    if (!evt)
        return res.status(404).json({ message: 'Evento não encontrado' });
    if (!isAdmin && evt.bibliotecaId !== myBibId)
        return res.status(403).json({ message: 'Sem permissão' });
    const rows = await prisma_1.prisma.eventoParticipante.findMany({
        where: { eventoId },
        orderBy: { id: 'asc' },
        include: {
...
r.delete('/:id/inscricoes/:participanteId', (0, auth_1.auth)(), (0, auth_1.requireRole)(client_1.Role.ADMIN, client_1.Role.BIBLIOTECARIO), (0, async_1.asyncHandler)(async (req, res) => {
    const { isAdmin, bibliotecaId: myBibId } = await getUserScope(req);
    const eventoId = zod_1.z.coerce.number().int().positive().parse(req.params.id);
    const participanteId = zod_1.z.coerce.number().int().positive().parse(req.params.participanteId);
    const p = await prisma_1.prisma.eventoParticipante.findUnique({
    ...
    await prisma_1.prisma.eventoParticipante.delete({ where: { id: participanteId } });
    res.status(204).send();
}));
/* =========================== Minhas inscrições =========================== */
r.get('/minhas-inscricoes', (0, auth_1.auth)(), (0, auth_1.requireRole)(client_1.Role.PAI, client_1.Role.BIBLIOTECARIO, client_1.Role.ADMIN), (0, async_1.asyncHandler)(async (_req, res) => {
    const userId = _req.auth.userId;
    const parts = await prisma_1.prisma.eventoParticipante.findMany({
...
/* =========================== Presença (self) =========================== */
r.post('/:id/presenca/self', (0, auth_1.auth)(), (0, auth_1.requireRole)(client_1.Role.PAI), (0, async_1.asyncHandler)(async (req, res) => {
    const eventoId = zod_1.z.coerce.number().int().positive().parse(req.params.id);
    const userId = req.auth.userId;
    const evt = await prisma_1.prisma.evento.findUnique({ where: { id: eventoId } });
    if (!evt)
    ...
    const famId = await resolveFamiliaIdForUser(userId);
    const whereMine = famId ? { eventoId, familiaId: famId } : { eventoId, utilizadorId: userId };
    const part = await prisma_1.prisma.eventoParticipante.findFirst({ where: whereMine });
    if (!part)
        return res.status(404).json({ message: 'Inscrição não encontrada para marcar presença.' });
    if (part.presente)
        return res.status(200).json({ ok: true, presente: true });
    await prisma_1.prisma.eventoParticipante.update({ where: { id: part.id }, data: { presente: true } });
    const presentes = await prisma_1.prisma.eventoParticipante.count({ where: { eventoId, presente: true } });
