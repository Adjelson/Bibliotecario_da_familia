...
/* ============================ Zod ============================ */
const MetodoEnum = zod_1.z.preprocess((v) => (typeof v === 'string' ? v.toUpperCase() : v), zod_1.z.enum(['PRESENCIAL', 'VIDEO']));
const CreateBody = zod_1.z.object({
    dataHora: zod_1.z.coerce.date(),
    bibliotecarioId: zod_1.z.number().int().positive(),
    notas: zod_1.z.string().trim().optional(),
    metodo: MetodoEnum.optional(),
    familiaId: zod_1.z.number().int().positive().optional(), // ADMIN pode marcar para outra família
});
const UpdateBody = zod_1.z.object({
    dataHora: zod_1.z.coerce.date().optional(),
    bibliotecarioId: zod_1.z.number().int().positive().optional(),
    notas: zod_1.z.string().trim().optional(),
    status: zod_1.z.enum(['MARCADA', 'RECUSADA', 'RETORNADA', 'CONCLUIDA', 'CANCELADA']).optional(),
    motivo: zod_1.z.string().trim().optional(),
    resultadoResumo: zod_1.z.string().trim().optional(),
    enviarResultadoAgora: zod_1.z.coerce.boolean().optional(),
});
const ResponderBody = zod_1.z.object({
    info: zod_1.z.string().trim().min(1),
});
const asDate = zod_1.z.preprocess((v) => (v === '' || v === null || v === undefined ? undefined : v), zod_1.z.coerce.date().optional());
const ListQuery = zod_1.z.object({
    ...
    desde: asDate,
    ate: asDate,
    bibliotecarioId: zod_1.z.coerce.number().int().positive().optional(),
    familiaId: zod_1.z.coerce.number().int().positive().optional(),
});
/* ========================== helpers ========================== */
function toDto(c) {
    return {
        ...
        familiaFilhos: Array.isArray(c.familia?.filhos)
            ? c.familia.filhos.map((f) => ({ id: f.id, nome: f.nome, idade: f.idade, genero: f.genero, perfilLeitor: f.perfilLeitor }))
            : [],
    };
}
function addMinutes(dt, m) { return new Date(dt.getTime() + m * 60 * 1000); }
function overlaps(aStart, aEnd, bStart, bEnd) { return aStart < bEnd && bStart < aEnd; }
/** Construção sempre em UTC para evitar “escorregar” o dia noutras TZs */
function dateAtUTC(y, m, d, hh, mm) {
...
function fmtPt(dt) {
    try {
        return dt.toLocaleString('pt-PT', { dateStyle: 'medium', timeStyle: 'short' });
    }
    catch {
...
function makeETag(payload) {
    const hash = crypto_1.default.createHash('sha1').update(JSON.stringify(payload)).digest('hex');
    return `W/"${hash}"`;
}
async function notifyMany(data) {
    const payload = data
    ...
    if (payload.length) {
        await prisma_1.prisma.notificacao.createMany({ data: payload, skipDuplicates: true });
    }
}
async function getUserBibId(userId) {
    const u = await prisma_1.prisma.user.findUnique({ where: { id: userId }, select: { bibliotecaId: true } });
    return u?.bibliotecaId ?? null;
}
async function getBibliotecarioBibId(bibliotecarioId) {
    const bib = await prisma_1.prisma.user.findUnique({ where: { id: bibliotecarioId }, select: { bibliotecaId: true, role: true, isActive: true } });
    if (!bib || bib.role !== client_1.Role.BIBLIOTECARIO || !bib.isActive)
        return null;
    return bib.bibliotecaId ?? null;
}
/* =================== Disponibilidade =================== */
r.get('/disponibilidade', (0, auth_1.auth)(true), (0, async_1.asyncHandler)(async (req, res) => {
    const bibliotecarioId = zod_1.z.coerce.number().int().positive().parse(req.query.bibliotecarioId);
    const dias = zod_1.z.coerce.number().int().min(1).max(60).default(14).parse(req.query.dias ?? 14);
    // Gate por biblioteca:
    const targetBibId = await getBibliotecarioBibId(bibliotecarioId);
    if (!targetBibId)
        return res.status(404).json({ message: 'Bibliotecário inválido' });
    const authCtx = req.auth;
    if (authCtx?.role === client_1.Role.BIBLIOTECARIO && authCtx.userId !== bibliotecarioId) {
        return res.status(403).json({ message: 'Só pode consultar a sua própria disponibilidade' });
    }
    if (authCtx?.role === client_1.Role.PAI) {
        const myBib = await getUserBibId(authCtx.userId);
        if (!myBib || myBib !== targetBibId)
            return res.status(403).json({ message: 'Bibliotecário de outra biblioteca' });
    }
    // Limites (UTC)
    const qDesde = req.query.desde ? new Date(String(req.query.desde)) : null;
    const qAte = req.query.ate ? new Date(String(req.query.ate)) : null;
    const now = new Date();
    ...
    for (const c of consultas) {
        ...
        if (!consultasPorDia.has(k))
            consultasPorDia.set(k, []);
    ...
    for (let d = new Date(start); d <= end; d = addMinutes(d, 24 * 60)) {
        ...
        const slotsSemBloqueio = slotsDia.filter(({ dt, step }) => !bloqueios.some(b => overlaps(dt, addMinutes(dt, step), b.inicio, b.fim)));
        // Remove slots já ocupados (±30min)
        const booked = consultasPorDia.get(d.toISOString().slice(0, 10)) ?? [];
        const slotsLivres = slotsSemBloqueio.filter(({ dt }) => !booked.some(cdt => Math.abs(dt.getTime() - cdt.getTime()) < 30 * 60 * 1000));
...
r.get('/bibliotecarios', (0, auth_1.auth)(true), (0, async_1.asyncHandler)(async (req, res) => {
    const qBib = Number(req.query.bibliotecaId ?? NaN);
    let bibliotecaId = Number.isFinite(qBib) ? qBib : null;
    const authCtx = req.auth;
    if (authCtx?.role === client_1.Role.PAI || authCtx?.role === client_1.Role.BIBLIOTECARIO) {
        bibliotecaId = await getUserBibId(authCtx.userId);
        if (!bibliotecaId)
            return res.json([]);
    }
    const where = { role: client_1.Role.BIBLIOTECARIO, isActive: true };
    if (bibliotecaId)
...
r.get('/familias', (0, auth_1.auth)(), (0, auth_1.requireRole)(client_1.Role.PAI, client_1.Role.BIBLIOTECARIO, client_1.Role.ADMIN), (0, async_1.asyncHandler)(async (req, res) => {
    ...
    else if (authCtx.role === client_1.Role.BIBLIOTECARIO) {
        const myBib = await getUserBibId(authCtx.userId);
        if (!myBib)
            return res.json([]);
...
/* =============================== CRUD =============================== */
// Criar
r.post('/', (0, auth_1.auth)(), (0, auth_1.requireRole)(client_1.Role.PAI, client_1.Role.ADMIN, client_1.Role.BIBLIOTECARIO), (0, async_1.asyncHandler)(async (req, res) => {
    ...
    let familiaId = null;
    if (authCtx.role === client_1.Role.ADMIN) {
        familiaId = data.familiaId ?? null;
    }
    else if (authCtx.role === client_1.Role.PAI) {
        const fam = await prisma_1.prisma.familia.findUnique({ where: { userId: authCtx.userId } });
        if (!fam)
        ...
        if (!data.familiaId)
            return res.status(400).json({ message: 'Família é obrigatória' });
        familiaId = data.familiaId;
    }
    if (!familiaId)
        return res.status(400).json({ message: 'Família é obrigatória' });
    // Bibliotecário válido e ativo
    const bibUser = await prisma_1.prisma.user.findUnique({
        where: { id: data.bibliotecarioId },
        select: { id: true, role: true, isActive: true, bibliotecaId: true },
    });
    if (!bibUser || bibUser.role !== client_1.Role.BIBLIOTECARIO || !bibUser.isActive) {
        return res.status(400).json({ message: 'Bibliotecário inválido' });
    }
    // Gate por biblioteca
    if (authCtx.role === client_1.Role.PAI) {
        const familiaUser = await prisma_1.prisma.user.findUnique({
        ...
        const famWithUser = await prisma_1.prisma.familia.findUnique({
            where: { id: familiaId },
    ...
    try {
        const created = await prisma_1.prisma.$transaction(async (tx) => {
            ...
            if (conflito)
                throw Object.assign(new Error('CONFLITO'), { http: 409 });
            ...
            if (bloqueio)
                throw Object.assign(new Error('BLOQUEADO'), { http: 409 });
            // Montagem de notas/metodologia
            const PENDENTE_TAG = '[PENDENTE]';
            const metodoUpper = (data.metodo ?? undefined);
...
r.get('/', (0, auth_1.auth)(), (0, async_1.asyncHandler)(async (req, res) => {
    ...
    if (role === client_1.Role.PAI) {
        const me = await prisma_1.prisma.user.findUnique({ where: { id: userId }, select: { bibliotecaId: true } });
        const fam = await prisma_1.prisma.familia.findUnique({ where: { userId }, select: { id: true } });
        where.familiaId = fam?.id ?? -1;
        if (me?.bibliotecaId)
            where.bibliotecario = { is: { bibliotecaId: me.bibliotecaId } };
    }
    else if (role === client_1.Role.BIBLIOTECARIO) {
        where.bibliotecarioId = userId;
        const myBib = await getUserBibId(userId);
        if (myBib)
            where.bibliotecario = { is: { bibliotecaId: myBib } };
    }
    else {
        const qBib = Number(req.query.bibliotecaId ?? NaN);
        if (Number.isFinite(qBib))
            where.bibliotecario = { is: { bibliotecaId: qBib } };
        if (q.familiaId)
            where.familiaId = q.familiaId;
        if (q.bibliotecarioId)
            where.bibliotecarioId = q.bibliotecarioId;
    }
    if (q.status)
    ...
    const [total, items, maxCreated, maxUpdated] = await Promise.all([
        prisma_1.prisma.consulta.count({ where }),
        prisma_1.prisma.consulta.findMany({
            where,
        ...
        prisma_1.prisma.consulta.aggregate({ _max: { createdAt: true }, where }).then(a => a._max?.createdAt ?? null),
        prisma_1.prisma.consulta.aggregate({ _max: { updatedAt: true }, where }).then(a => a._max?.updatedAt ?? null),
    ]);
    const etag = makeETag({
        total,
...
r.get('/:id', (0, auth_1.auth)(), (0, async_1.asyncHandler)(async (req, res) => {
    const id = zod_1.z.coerce.number().int().parse(req.params.id);
    const authCtx = req.auth;
    const c = await prisma_1.prisma.consulta.findUnique({
        where: { id },
        include: {
            familia: { include: { user: true, filhos: { select: { id: true, nome: true, idade: true, genero: true, perfilLeitor: true } } } },
            bibliotecario: { select: { id: true, name: true, email: true, bibliotecaId: true } },
    ...
    if (authCtx.role === client_1.Role.PAI) {
        const fam = await prisma_1.prisma.familia.findUnique({ where: { userId: authCtx.userId }, select: { id: true } });
        if (c.familiaId !== (fam?.id ?? -1))
            return res.status(403).json({ message: 'Sem acesso' });
    }
    else if (authCtx.role === client_1.Role.BIBLIOTECARIO && c.bibliotecarioId !== authCtx.userId) {
        return res.status(403).json({ message: 'Sem acesso' });
    }
    res.json(toDto(c));
}));
...
r.patch('/:id', (0, auth_1.auth)(), (0, auth_1.requireRole)(client_1.Role.ADMIN, client_1.Role.BIBLIOTECARIO), (0, async_1.asyncHandler)(async (req, res) => {
    const id = zod_1.z.coerce.number().int().parse(req.params.id);
    const body = UpdateBody.parse(req.body);
    const authCtx = req.auth;
    const current = await prisma_1.prisma.consulta.findUnique({
        where: { id },
        include: {
    ...
    if (authCtx.role === client_1.Role.BIBLIOTECARIO && current.bibliotecarioId !== authCtx.userId) {
        return res.status(403).json({ message: 'Sem acesso' });
    }
    ...
    if (body.dataHora) {
        ...
        const conflito = await prisma_1.prisma.consulta.findFirst({
            where: {
                id: { not: id },
        ...
        if (conflito)
            return res.status(409).json({ message: 'Já existe consulta marcada nesse período.' });
    }
    // Reatribuição: validar bibliotecário
    if (body.bibliotecarioId && body.bibliotecarioId !== current.bibliotecarioId) {
        const novoBib = await prisma_1.prisma.user.findUnique({
            where: { id: body.bibliotecarioId },
            select: { id: true, role: true, isActive: true, bibliotecaId: true },
        });
        if (!novoBib || novoBib.role !== client_1.Role.BIBLIOTECARIO || !novoBib.isActive) {
            return res.status(400).json({ message: 'Bibliotecário inválido' });
        }
        // BIB não pode reatribuir para outros
        if (authCtx.role === client_1.Role.BIBLIOTECARIO) {
    ...
    if (body.notas !== undefined)
        dataUpdate.notas = body.notas || null;
    if (body.dataHora)
        dataUpdate.dataHora = body.dataHora;
    if (body.bibliotecarioId)
        dataUpdate.bibliotecario = { connect: { id: body.bibliotecarioId } };
    ...
    if (body.status) {
        ...
        if (body.status === 'RECUSADA') {
            if (!body.motivo?.trim())
                return res.status(422).json({ message: 'Motivo é obrigatório para recusa' });
            dataUpdate.recusaMotivo = body.motivo.trim();
            if (familiaUserId)
                notify.push({ userId: familiaUserId, type: 'CONSULTA_RECUSADA', title: 'Consulta recusada', body: body.motivo.trim() });
        }
        if (body.status === 'RETORNADA') {
            if (!body.motivo?.trim())
                return res.status(422).json({ message: 'Motivo é obrigatório para pedir informação' });
            dataUpdate.retornoMotivo = body.motivo.trim();
            if (familiaUserId)
                notify.push({ userId: familiaUserId, type: 'CONSULTA_INFO', title: 'Informação adicional necessária', body: body.motivo.trim() });
        }
        ...
        if (body.status === 'CANCELADA') {
            if (familiaUserId)
                notify.push({ userId: familiaUserId, type: 'CONSULTA_CANCELADA', title: 'Consulta cancelada' });
        }
    }
    const updated = await prisma_1.prisma.consulta.update({
        where: { id },
        data: dataUpdate,
        include: {
            familia: { include: { user: true } },
...
r.post('/:id/responder', (0, auth_1.auth)(), (0, auth_1.requireRole)(client_1.Role.PAI, client_1.Role.ADMIN), (0, async_1.asyncHandler)(async (req, res) => {
    const id = zod_1.z.coerce.number().int().parse(req.params.id);
...
r.post('/:id/cancelar', (0, auth_1.auth)(), (0, auth_1.requireRole)(client_1.Role.PAI, client_1.Role.BIBLIOTECARIO, client_1.Role.ADMIN), (0, async_1.asyncHandler)(async (req, res) => {
    const id = zod_1.z.coerce.number().int().parse(req.params.id);
...
// Remover (ADMIN)
r.delete('/:id', (0, auth_1.auth)(), (0, auth_1.requireRole)(client_1.Role.ADMIN), (0, async_1.asyncHandler)(async (req, res) => {
    const id = zod_1.z.coerce.number().int().parse(req.params.id);
    await prisma_1.prisma.consulta.delete({ where: { id } });
    res.json({ ok: true });
}));
exports.default = r;
